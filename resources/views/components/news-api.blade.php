        ...
        async loadNews() {
            try {
                ...
                if (result.success && result.data && result.data.length > 0) {
                    // Successfully got real data from API
                    this.displayNews(result.data[0]);
                } else if (result.fallback) {
                    // API returned fallback data (this is still valid data!)
                    this.displayNews(result.fallback);
                    // Don't show error - fallback data is intentional and valid
                } else {
            ...
            for (const endpoint of endpoints) {
                try {
                    ...
                    if (response.ok && data.success) {
                        ...
                        return data;
                    } else if (response.ok && data.fallback) {
                        // API returned fallback data (this is still valid!)
                        return {
                            success: true,
                            data: [data.fallback],
                            message: 'Using fallback health tip'
                        };
                    }
                } catch (error) {
